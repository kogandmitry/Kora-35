...
test('builds detailed and customer budget views', () => {
  ...
  assert.equal(org.total, 185);
  assert.equal(org.reserve, 30);
  assert.equal(org.lines.length, 3);
  assert.equal(org.scenario.label, 'Общая смета проекта');
  assert.equal(ignoredLegacyScenario.total, 185);
  assert.deepEqual(org.unestimatedLines, []);
  assert.equal(customer.lines.length, 3);
  assert.equal(customer.potentialLines.length, 1);
  assert.deepEqual(customer.unestimatedLines, []);
  assert.equal(customer.total, 185);
